...
def calcular_estadisticas() -> EstadisticasResponse:
    ...
    - Promedio de satisfacción con el gobierno por departamento (pregunta P_GOBIERNO)
    """
    ...
    if total == 0:
        return EstadisticasResponse(
            ...
            satisfaccion_gobierno_por_departamento={},
        )
    ...
    # ── Promedio de satisfacción gobierno por departamento ──────────────────
    # Recorre todas las encuestas buscando la respuesta con pregunta_id='P_GOBIERNO'
    # y acumula los valores Likert por departamento para calcular el promedio.
    acum_satisfaccion: dict[str, list[int]] = {}
    for enc in encuestas:
        depto = enc.encuestado.departamento
        resp_gobierno = next(
            (r for r in enc.respuestas if r.pregunta_id == "P_GOBIERNO"),
            None
        )
        if resp_gobierno is not None and isinstance(resp_gobierno.respuesta, int):
            acum_satisfaccion.setdefault(depto, []).append(resp_gobierno.respuesta)

    satisfaccion_por_depto = {
        depto: round(sum(vals) / len(vals), 2)
        for depto, vals in acum_satisfaccion.items()
    }
    ...
    return EstadisticasResponse(
        ...
        satisfaccion_gobierno_por_departamento=satisfaccion_por_depto,
    )
